...
## v0.9.5（RAG 扩量评测 + MCP Client 接入）

> 日期：2026-08-30

### ✨ RAG 评测扩量
- 知识库 36 → 57 篇（新增 LLM 基础 / Agent 深入 / 后端基础 / 算法 / 安全）
- 评测集 36 → 56 条（新增语义改写难题 / 近义干扰难题 / 跨主题多文档）
- 指标重构：主表从 recall@5 换成 recall@1 + MRR + NDCG（recall@5 在小库下饱和无区分度）
- 拒答阈值校准：正负例相似度分布在 0.40 处干净分离（误拒率 0%、漏挡率 0%）
- 实测：混合检索 recall@1 76.4%→82.1%、MRR 92.1%→95.3%、语义改写难题 60%→90%

### ✨ MCP Client 接入
- 新增 `mcp/client.py`：MCPToolAdapter + MCPClientManager，接入 GitHub MCP Server（stdio）
- **只读白名单**：26 个工具中过滤 12 个写操作，只注册 14 个只读工具（不把写能力交给 LLM）
- async/sync 桥接：独立线程 + 独立事件循环，避开 uvicorn 嵌套事件循环
- 状态机加外部意图检测，本地 RAG + 外部工具分层

### 🧪 测试补强
- 后端测试 107 → 115 个（新增 test_mcp_client.py 8 个）
